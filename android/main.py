...
import os, json, math, time, urllib.request, urllib.parse, threading
...
def yahoo_chart(ticker, range_="2y"):
    ...
    urls = [
        f"https://query1.finance.yahoo.com/v8/finance/chart/{t}?range={range_}&interval=1d&events=history&includeAdjustedClose=false",
        f"https://query2.finance.yahoo.com/v8/finance/chart/{t}?range={range_}&interval=1d&events=history&includeAdjustedClose=false",
    ]
    last = None
    headers = {"User-Agent": "Mozilla/5.0 (Android) QuantScanner/2.0"}
    for url in urls:
        try:
            ...
            if not result:
                raise ValueError("Yahoo boş sonuç")
            x = result[0]
            ...
            if len(rows) < 220:
                raise ValueError(f"Yetersiz veri: {len(rows)}")
            return rows
